...
use morphorm::{GeometryChanged, Cache};

use crate::{KayakContext, Index, KayakContextRef, LayoutEvent};
    ...
    fn process(index: Index, context: &mut KayakContext) {
        ...
        if let Some(on_layout) = widget.get_props().get_on_layout() {
          if let Some(rect) = context.widget_manager.layout_cache.rect.get(&index) {  
            ...
            let mut layout_event = LayoutEvent::new(*rect, context.widget_manager.layout_cache.geometry_changed(index) ,  index);
