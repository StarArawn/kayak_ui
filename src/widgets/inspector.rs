...
pub fn Inspector() {
    ...
    let last_clicked = last_clicked.get();
    let (id, _) = last_clicked.into_raw_parts();

    if let Some(layout) = context.widget_manager.get_layout(&last_clicked) {
        if let Some(_node) = context.widget_manager.get_node(&last_clicked) {
            let mut data = Vec::new();

            if let Some(name) = context.widget_manager.get_name(&last_clicked) {
            ...
            if let Some(parent_id) = context.widget_manager.get_valid_parent(last_clicked) {
